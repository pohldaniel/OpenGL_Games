...
float Tower::TileSize = 64.0f;

Tower::Tower(StateMachine& machine) : State(machine, States::TOWER), m_level(m_camera, static_cast<int>(static_cast<float>(Application::Width) / TileSize), static_cast<int>(static_cast<float>(Application::Height) / TileSize)),
	...
	if (!TileSetManager::Get().containsTileset("sprites")) {
...
void Tower::updateUnits(float dt) {
...
void Tower::OnMouseMotion(Event::MouseMoveEvent& event) {
	m_mouseX = event.x;
	m_mouseY = event.y;
} 

void Tower::OnMouseButtonDown(Event::MouseButtonEvent& event) {
...
void Tower::OnMouseButtonUp(Event::MouseButtonEvent& event) {
	m_mouseDownLeft = false;
	m_mouseDownRight = false;
}

void Tower::OnMouseWheel(Event::MouseWheelEvent& event) {
	
}

void Tower::OnKeyDown(Event::KeyboardEvent& event) {
...
void Tower::OnKeyUp(Event::KeyboardEvent& event) {

}

void Tower::resize(int deltaW, int deltaH) {
	m_camera.perspective(45.0f, static_cast<float>(Application::Width) / static_cast<float>(Application::Height), 0.1f, 1000.0f);
	m_camera.orthographic(0.0f, static_cast<float>(Application::Width), 0.0f, static_cast<float>(Application::Height), -1.0f, 1.0f);
}
...
void Tower::addUnit(const Vector2f& posMouse) {
	listUnits.push_back(std::make_shared<Unit>(posMouse));
}

void Tower::addTurret(const Vector2f& posMouse) {
	Vector2f pos((int)posMouse[0] + 0.5f, (int)posMouse[1] + 0.5f);
	listTurrets.push_back(Turret(pos));
}

void Tower::removeTurretsAtMousePosition(const Vector2f& posMouse) {
...
void Tower::updateSpawnUnitsIfRequired(float dt) {
