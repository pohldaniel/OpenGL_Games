...
LevelFlow::LevelFlow(const Camera& _camera, int setTileCountX, int setTileCountY) : camera(_camera), tileCountX(setTileCountX), tileCountY(setTileCountY) {
	size_t listTilesSize = (size_t)tileCountX * tileCountY;
	m_tiles.assign(listTilesSize, TileFlow());
...
void LevelFlow::init(std::vector<TextureRect>& textureRects) {
	m_rextureRects = textureRects;
}

void LevelFlow::draw(float size) {
	...
	for (auto& tile : m_tiles) {
		...
		TileFlow& tileSelected = m_tiles[x + y * tileCountX];
...
bool LevelFlow::isTileWall(int x, int y) {
...
void LevelFlow::setTileWall(int x, int y, bool setWall) {
	int index = x + y * tileCountX;
	if (index > -1 && index < m_tiles.size() &&
		x > -1 && x < tileCountX &&
		y > -1 && y < tileCountY)
...
Vector2f LevelFlow::getTargetPos() {
	return Vector2f((float)targetX, (float)targetY);
}


void LevelFlow::setTargetAndCalculateFlowField(int targetXNew, int targetYNew) {
...
void LevelFlow::calculateDistances() {
	int indexTarget = targetX + targetY * tileCountX;
...
void LevelFlow::calculateFlowDirections() {
	//The offset of the neighboring tiles to be checked.
...
Vector2f LevelFlow::getFlowNormal(int x, int y) {
	int index = x + y * tileCountX;
	if (index > -1 && index < m_tiles.size() &&
		x > -1 && x < tileCountX &&
		y > -1 && y < tileCountY)
