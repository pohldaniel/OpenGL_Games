...
void LevelManager::LoadCarImage(std::string& imageName)
{
	...
	//load
	Texture2* image = TextureManager::Instance()->LoadFromFile(imageLocation);

	if (image != 0)
	{
		//add to collection
		_images.insert(std::pair<std::string, Texture2*>(imageName, image));
	}
}
...
Texture2* LevelManager::GetThumb(std::string levelName)
{
	std::string thumb = "";
...
Texture2* LevelManager::GetCarImage(std::string carImage)
{
...
bool LevelManager::LoadLevel(World *world, std::string levelName, std::string carFileName)
{
	...
	//loac main level file
	BaseFile* file = FileManager::Instance()->GetFile(levelFile);
...
{
	//loac main level file
	BaseFile* file = FileManager::Instance()->GetFile(fileName,true);
...
{
	std::string saveFile = FileManager::Instance()->GetSaveDirPath() + fileName;
...
void LevelManager::ReadLevelData(std::string levelName)
{
	...
	//test save file
	{
		...
		BaseFile* saveFile = FileManager::Instance()->GetFile(levelFile);

		if (saveFile == 0)
			return;

		saveFile->Open(Write, Binary);
...
{
	//loac main level file
	BaseFile* file = FileManager::Instance()->GetFile(bodyName);
