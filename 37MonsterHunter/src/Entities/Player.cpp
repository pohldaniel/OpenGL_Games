...
bool Player::hasLineOfSight(const Cell& _cell, const Rect& collisionRect, float radius) const {
	if (Vector2f::Length(Vector2f(_cell.centerX, _cell.centerY), Vector2f(cell.centerX, cell.centerY)) < radius) {
		for (const Rect& rect : collisionRects) {
			if (&collisionRect == &rect) 
				continue;
			
			if (lineRect(_cell.centerX, _cell.centerY, cell.centerX, cell.centerY, rect.posX, rect.posY, rect.width, rect.height)) {
				return false;
			}
		}
	}
