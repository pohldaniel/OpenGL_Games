...
MonsterIndex::MonsterIndex() : m_visibleItems(6), m_viewWidth(0.0f), m_viewHeight(0.0f), m_currentOffset(0), m_currentSelected(0), m_beforeSelected(-1){
	m_names.push_back({ "Ivieron", 9u, false });
	m_names.push_back({ "Atrox", 0u, false });
	m_names.push_back({ "Cindrill", 2u, false });
	m_names.push_back({ "Atrox", 0u, false });
	m_names.push_back({ "Sparchu", 15u, false });
	m_names.push_back({ "Gulfin", 8u, false });
	m_names.push_back({ "Jacana", 10u, false });
	m_names.push_back({ "Plumette", 13u, false });
	m_names.push_back({ "Cleaf", 3u, false });
...
void MonsterIndex::processInput() {
	...
	if (keyboard.keyPressed(Keyboard::KEY_SPACE)) {
		std::get<2>(m_names[m_currentOffset + m_currentSelected]) = !std::get<2>(m_names[m_currentOffset + m_currentSelected]);
		if (m_beforeSelected != m_currentOffset + m_currentSelected) {
			if (m_beforeSelected != -1) {
				std::get<2>(m_names[m_currentOffset + m_currentSelected]) = false;
				std::get<2>(m_names[m_beforeSelected]) = false;
