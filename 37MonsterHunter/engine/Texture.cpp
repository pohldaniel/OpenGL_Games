...
void Texture::loadFromFileGpu() {
	...
	if (m_target == GL_TEXTURE_CUBE_MAP) {
		...
		// load face data
		for (int i = 0; i < 6; i++) {
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, m_internalFormat, m_width, m_height, 0, m_format, m_type, faceData[i]);
		}
...
void Texture::loadCrossCubeFromFileCpu(std::string fileName, const bool _flipVertical, unsigned int _internalFormat, unsigned int _format) {
	...
	for (int i = 0; i < 6; i++) {
		if (_flipVertical && i != 2)
			flipVertical(faceData[i], m_channels * m_width, m_height);
	}
}
