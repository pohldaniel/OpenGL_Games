...
std::tuple<double, double> GetEntityVelocity(Entity entity) {
    if (entity.HasComponent<RigidBodyComponent>()) {
		...
		std::cout << "Trying to get the velocity of an entity that has no rigidbody component" << std::endl;
        return std::make_tuple(0.0, 0.0);
    }
}
...
void SetEntityPosition(Entity entity, double x, double y) {
    if (entity.HasComponent<TransformComponent>()) {
		...
		std::cout << "Trying to set the position of an entity that has no transform component" << std::endl;
    }
}
...
void SetEntityVelocity(Entity entity, double x, double y) {
    if (entity.HasComponent<RigidBodyComponent>()) {
		...
		std::cout << "Trying to set the velocity of an entity that has no rigidbody component" << std::endl;
    }
}
...
void SetEntityRotation(Entity entity, double angle) {
    if (entity.HasComponent<TransformComponent>()) {
		...
		std::cout << "Trying to set the rotation of an entity that has no transform component" << std::endl;
    }
}
...
void SetEntityAnimationFrame(Entity entity, int frame) {
    if (entity.HasComponent<AnimationComponent>()) {
		...
		std::cout << "Trying to set the animation frame of an entity that has no animation component" << std::endl;
    }
}
...
void SetProjectileVelocity(Entity entity, double x, double y) {
    if (entity.HasComponent<ProjectileEmitterComponent>()) {
		...
		std::cout << "Trying to set the projectile velocity of an entity that has no projectile emitter component" << std::endl;
    }
}
