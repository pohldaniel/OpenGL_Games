...
class ProjectileEmitSystem: public System {
    public:
        ...
        void OnKeyPressed(KeyPressedEvent& event) {
            if (event.symbol == Keyboard::KEY_SPACE) {
                for (auto entity: GetSystemEntities()) {
                    if (entity.HasTag("player")) {
                        ...
                        int directionX = 0;
                        int directionY = 0;
                        if (keyboardcontrol.direction == Direction::EAST) directionX = +1;
                        if (keyboardcontrol.direction == Direction::WEST) directionX = -1;
                        if (keyboardcontrol.direction == Direction::NORTH) directionY = +1;
                        if (keyboardcontrol.direction == Direction::SOUTH) directionY = -1;
        ...
        void Update(std::unique_ptr<Registry>& registry) {
            for(auto entity: GetSystemEntities()) {
                ...
                if(Globals::clock.getElapsedTimeMilli() - projectileEmitter.lastEmissionTime > projectileEmitter.repeatFrequency) {
                    glm::vec2 projectilePosition = transform.position;
                    if (entity.HasComponent<SpriteComponent>()) {
                    ...
                    projectileEmitter.lastEmissionTime = Globals::clock.getElapsedTimeMilli();
                }
            }
        }
};
