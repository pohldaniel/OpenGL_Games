...
SolidState::SolidState(StateMachine& machine) : State(machine, States::SOLID) {
	...
	solidConverter.loadAnimation("res/animations/Tempanim", Utils::lowheight, Utils::neutral, tempanimation);
...
void SolidState::doAnimations(float dt) {
	if (!m_skeleton.free) {
		...
		if (target >= 1) {
			...
			if ((animCurrent == Utils::walljumprightkickanim && animTarget == Utils::walljumprightkickanim) || (animCurrent == Utils::walljumpleftkickanim && animTarget == Utils::walljumpleftkickanim)) {
				XYZ rotatetarget = DoRotation(m_skeleton.forward, 0, yaw, 0);
		...
		if (!m_skeleton.free) {
			...
			if (animCurrent != oldanimCurrent || animTarget != oldanimTarget || ((frameCurrent != oldframeCurrent || frameTarget != oldframeTarget) && !calcrot)) {
				...
				for (unsigned i = 0; i < m_skeleton.joints.size(); i++) {
					//m_skeleton.joints[i].position = currentFrame().joints[i].position;
				...
				m_skeleton.FindForwards();

				for (unsigned i = 0; i < m_skeleton.muscles.size(); i++) {
					if (m_skeleton.muscles[i].visible) {
						m_skeleton.FindRotationMuscle(i, animTarget);
					}
				}
				for (unsigned i = 0; i < m_skeleton.muscles.size(); i++) {
					if (m_skeleton.muscles[i].visible) {
						if (isnormal((float)((int)(m_skeleton.muscles[i].rotate1 * 100) % 36000) / 100)) {
							m_skeleton.muscles[i].oldrotate1 = (float)((int)(m_skeleton.muscles[i].rotate1 * 100) % 36000) / 100;
						}
						if (isnormal((float)((int)(m_skeleton.muscles[i].rotate2 * 100) % 36000) / 100)) {
							m_skeleton.muscles[i].oldrotate2 = (float)((int)(m_skeleton.muscles[i].rotate2 * 100) % 36000) / 100;
						}
						if (isnormal((float)((int)(m_skeleton.muscles[i].rotate3 * 100) % 36000) / 100)) {
							m_skeleton.muscles[i].oldrotate3 = (float)((int)(m_skeleton.muscles[i].rotate3 * 100) % 36000) / 100;
						}
					}
				}
				...
				for (unsigned i = 0; i < m_skeleton.muscles.size(); i++) {
					if (m_skeleton.muscles[i].visible) {
						if (isnormal((float)((int)(m_skeleton.muscles[i].rotate1 * 100) % 36000) / 100)) {
							m_skeleton.muscles[i].newrotate1 = (float)((int)(m_skeleton.muscles[i].rotate1 * 100) % 36000) / 100;
						}
						if (isnormal((float)((int)(m_skeleton.muscles[i].rotate2 * 100) % 36000) / 100)) {
							m_skeleton.muscles[i].newrotate2 = (float)((int)(m_skeleton.muscles[i].rotate2 * 100) % 36000) / 100;
						}
						if (isnormal((float)((int)(m_skeleton.muscles[i].rotate3 * 100) % 36000) / 100)) {
							m_skeleton.muscles[i].newrotate3 = (float)((int)(m_skeleton.muscles[i].rotate3 * 100) % 36000) / 100;
						}
						if (m_skeleton.muscles[i].newrotate3 > m_skeleton.muscles[i].oldrotate3 + 180) {
							m_skeleton.muscles[i].newrotate3 -= 360;
						}
						if (m_skeleton.muscles[i].newrotate3 < m_skeleton.muscles[i].oldrotate3 - 180) {
							m_skeleton.muscles[i].newrotate3 += 360;
						}
						if (m_skeleton.muscles[i].newrotate2 > m_skeleton.muscles[i].oldrotate2 + 180) {
							m_skeleton.muscles[i].newrotate2 -= 360;
						}
						if (m_skeleton.muscles[i].newrotate2 < m_skeleton.muscles[i].oldrotate2 - 180) {
							m_skeleton.muscles[i].newrotate2 += 360;
						}
						if (m_skeleton.muscles[i].newrotate1 > m_skeleton.muscles[i].oldrotate1 + 180) {
							m_skeleton.muscles[i].newrotate1 -= 360;
						}
						if (m_skeleton.muscles[i].newrotate1 < m_skeleton.muscles[i].oldrotate1 - 180) {
							m_skeleton.muscles[i].newrotate1 += 360;
						}
			...
			for (unsigned i = 0; i < m_skeleton.joints.size(); i++) {
				...
				m_skeleton.joints[i].velocity.x = vel[0];
				m_skeleton.joints[i].velocity.y = vel[1];
				m_skeleton.joints[i].velocity.z = vel[2];

				m_skeleton.joints[i].velocity -= m_skeleton.joints[i].position;
				m_skeleton.joints[i].velocity /= dt;

				Vector3f pos = currentFrame().joints[i].position * (1 - target) + targetFrame().joints[i].position * target;
				m_skeleton.joints[i].position.x = pos[0];
				m_skeleton.joints[i].position.y = pos[1];
				m_skeleton.joints[i].position.z = pos[2];
			}
			offset = currentoffset * (1 - target) + targetoffset * target;
			for (unsigned i = 0; i < m_skeleton.muscles.size(); i++) {
				if (m_skeleton.muscles[i].visible) {
					m_skeleton.muscles[i].rotate1 = m_skeleton.muscles[i].oldrotate1 * (1 - target) + m_skeleton.muscles[i].newrotate1 * target;
					m_skeleton.muscles[i].rotate2 = m_skeleton.muscles[i].oldrotate2 * (1 - target) + m_skeleton.muscles[i].newrotate2 * target;
					m_skeleton.muscles[i].rotate3 = m_skeleton.muscles[i].oldrotate3 * (1 - target) + m_skeleton.muscles[i].newrotate3 * target;
				}
			}
		}
