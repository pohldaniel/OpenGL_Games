...
static bool save_screenshot_png(const char* fname, int widht, int height);
...
bool save_screenshot(const char* file_name, int width, int height)
{
    const char* ptr = strrchr((char*)file_name, '.');
    if (ptr) {
        if (strcasecmp(ptr + 1, "png") == 0) {
            return save_screenshot_png((Folders::getScreenshotDir() + '/' + file_name).c_str(), width, height);
        }
    }
...
static bool save_screenshot_png(const char* file_name, int width, int height)
{
    FILE* fp = NULL;
    png_structp png_ptr = NULL;
    png_infop info_ptr = NULL;
    ...
    png_bytep screenshot = new png_byte[width * height * 3];
    ...
    for (int i = 0; i < height; i++) {
        row_pointers[i] = screenshot + ((width * ((height - 1) - i)) * 3);
    }
