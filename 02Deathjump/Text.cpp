...
Text::Text() {
	...
	projection = Matrix4f::GetOrthographic(projection, 0.f, static_cast<float>(WIDTH), 0.0f, static_cast<float>(HEIGHT), -1.0f, 1.0f);
	...
	// load font as face
	FT_Face face;
	if (FT_New_Face(ft, "res/fonts/upheavtt.ttf", 0, &face)) {
...
void Text::renderText(std::string text, float x, float y, float scale, Vector3f color) {

	glUseProgram(m_shaderText->m_program);
	m_shaderText->loadMatrix("projection", projection);
