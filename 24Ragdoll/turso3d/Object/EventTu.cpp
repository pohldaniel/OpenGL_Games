...
void EventTu::Send(RefCounted* sender)
{
#ifdef _DEBUG
    if (!IsMainThread())
    {
...
void EventTu::Subscribe(EventHandler* handler)
{
    if (!handler)
        return;
...
void EventTu::Unsubscribe(RefCounted* receiver)
{
    for (auto it = handlers.begin(); it != handlers.end(); ++it)
    {
        EventHandler* handler = *it;
...
bool EventTu::HasReceivers() const
{
    for (auto it = handlers.begin(); it != handlers.end(); ++it)
    {
        EventHandler* handler = *it;
...
bool EventTu::HasReceiver(const RefCounted* receiver) const
{
    for (auto it = handlers.begin(); it != handlers.end(); ++it)
    {
        EventHandler* handler = *it;
