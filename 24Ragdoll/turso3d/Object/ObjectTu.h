...
class ObjectTu : public RefCounted
{
public:
    ...
    void SubscribeToEvent(EventTu& event, EventHandler* handler);
    /// Unsubscribe from an event.
    void UnsubscribeFromEvent(EventTu& event);
    /// Send an event.
    void SendEvent(EventTu& event);
    ...
    bool SubscribedToEvent(const EventTu& event) const;
    ...
    static void RegisterSubsystem(ObjectTu* subsystem);
    /// Remove a subsystem by object pointer.
    static void RemoveSubsystem(ObjectTu* subsystem);
    ...
    static ObjectTu* Subsystem(StringHash type);
    ...
    static ObjectTu* Create(StringHash type);
    /// Destroy an object through a factory if exists, or if not, then by deleting.
    static void Destroy(ObjectTu* object);
...
private:
    ...
    static std::map<StringHash, ObjectTu*> subsystems;
...
public:
    ...
    virtual ObjectTu* Create() = 0;
    /// Destroy an object created through the factory.
    virtual void Destroy(ObjectTu* object) = 0;
...
public:
	...
	ObjectTu* Create() override { return allocator.Allocate(); }
    /// Destroy an object created through the factory.
    void Destroy(ObjectTu* object) override { return allocator.Free(static_cast<T*>(object)); }
