...
void DebugRenderer::SetView(CameraTu* camera)
{
    if (!camera)
        return;
...
void DebugRenderer::SetViewProjection(const Matrix4f& projection, const Matrix4f& view) {
...
void DebugRenderer::AddFrustum(const FrustumTu& frustum_, const Color& color, bool depthTest)
{
    unsigned startVertex = (unsigned)vertices.size();
    unsigned uintColor = color.ToUInt();
