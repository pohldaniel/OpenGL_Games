...
static TextureTu* boundTextures[MAX_TEXTURE_UNITS];
...
const unsigned TextureTu::glInternalFormats[] =
{
    0,
    GL_R8,
    GL_RG8,
...
void TextureTu::RegisterObject()
{
    RegisterFactory<TextureTu>();
}

bool TextureTu::BeginLoad(Stream& source)
{
    ZoneScoped;
...
bool TextureTu::Define(TextureType type_, const IntVector2& size_, ImageFormat format_, int multisample_, size_t numLevels_, const ImageLevel* initialData)
{
    return Define(type_, IntVector3(size_.x, size_.y, 1), format_, multisample_, numLevels_, initialData);
}

bool TextureTu::Define(TextureType type_, const IntVector3& size_, ImageFormat format_, int multisample_, size_t numLevels_, const ImageLevel* initialData)
{
    ZoneScoped;
...
bool TextureTu::DefineSampler(TextureFilterMode filter_, TextureAddressMode u, TextureAddressMode v, TextureAddressMode w, unsigned maxAnisotropy_, float minLod_, float maxLod_, const Color& borderColor_)
{
    ZoneScoped;
...
bool TextureTu::SetData(size_t level, const IntRect& rect, const ImageLevel& data)
{
    return SetData(level, IntBox(rect.left, rect.top, 0, rect.right, rect.bottom, 1), data);
}

bool TextureTu::SetData(size_t level, const IntBox& box, const ImageLevel& data)
{
    if (!texture)
        return true;
...
void TextureTu::Bind(size_t unit)
{
...
void TextureTu::Unbind(size_t unit)
{
...
unsigned TextureTu::GLTarget() const
{
    return glTargets[type];
}

void TextureTu::ForceBind()
{
    boundTextures[0] = nullptr;
    Bind(0);
}

void TextureTu::Release()
{
    if (texture)
    {
