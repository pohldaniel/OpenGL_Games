...
void TextOld::render(Vector4f color) {
	...
	for (c = m_label.begin(); c != m_label.end(); c++) {
		CharacterOld ch = m_characters[*c];
...
void TextOld::render(std::string text, Vector4f color) {
...
void TextOld::setPosition(const Vector2f &position) {
	m_position = position;
}

void TextOld::setOrigin(const Vector2f &origin) {
	m_origin = origin;
}

const Vector2f &TextOld::getPosition() const {
	return m_position;
}

const Vector2f &TextOld::getSize() const {
	return m_size;
}

void TextOld::calcSize() {
	int sizeX = 0;
	CharacterOld ch;
