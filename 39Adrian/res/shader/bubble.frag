...
float LinearizeDepth(float depth) {
	...
	float linearDepth = zNdc * (far - near) * 0.5 + (far + near) * 0.5;	
	return linearDepth;
}
...
void main() {
	...
	float bubbleDepth = gl_FragCoord.z;

	float distance = abs(sceneDepth - bubbleDepth) * scale; // linear difference in depth 
