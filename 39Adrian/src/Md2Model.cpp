...
void CMD2Model::LoadModel(char* sFilename)
{
	...
	vVertices.resize(header.num_frames, std::vector<glm::vec3>(header.num_xyz)); // Allocate space for vertices
	vNormals.resize(header.num_frames, std::vector<int>(header.num_xyz)); // And normals
	...
	if (index != -1)
	{
		...
		std::string sTry = sDirectory + sStripped + sTextureExtensions[0];
		tSkin.loadFromFile(sTry, true);
...
void CMD2Model::RenderModel(animState_t* animState)
{
	...
	if (animState == NULL)
	{
		...
		for (int i = 0; i < (int)renderModes.size(); i++)
		{
