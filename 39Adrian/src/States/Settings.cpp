...
Settings::Settings(StateMachine& machine) : State(machine, States::SETTINGS) {
	...
	m_seekerBars.at("effect").initButtons(Globals::fontManager.get("upheaval_30"));
	...
	m_seekerBars.at("music").initButtons(Globals::fontManager.get("upheaval_30"));
...
void Settings::render() {
	...
	Fontrenderer::Get().addText(Globals::fontManager.get("upheaval_30"), static_cast<float>(Application::Width / 2 - 465), static_cast<float>(Application::Height - 300), "Sound Volume", Vector4f(1.0f, 1.0f, 1.0f, 1.0f));
	Fontrenderer::Get().addText(Globals::fontManager.get("upheaval_30"), static_cast<float>(Application::Width / 2 - 465), static_cast<float>(Application::Height - 500), "Music Volume", Vector4f(1.0f, 1.0f, 1.0f, 1.0f));
	Fontrenderer::Get().addText(Globals::fontManager.get("upheaval_30"), static_cast<float>(Application::Width / 2 - 465), static_cast<float>(Application::Height - 700), m_checkBox.isChecked() ? "Use Skybox" : "Use procedural Clouds", Vector4f(1.0f, 1.0f, 1.0f, 1.0f));
