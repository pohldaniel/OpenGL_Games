...
private:
	...
	std::vector<std::vector<glm::vec3> > vVertices; // Vertices extracted for every frame
	std::vector <std::vector< int > > vNormals; // Normal indices extracted for every frame
	std::vector<int> glCommands; // Rendering OpenGL commands

	

	std::vector<int> renderModes; // Rendering modes
	std::vector<int> numRenderVertices; // with number of vertices
	unsigned int uiVAO;

	Texture tSkin;
	std::vector<CVertexBufferObject> vboFrameVertices; // All frames (keyframes) of model
