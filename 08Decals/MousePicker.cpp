...
MousePicker::MousePicker() {
	...
	m_mousePickBuffer.attachTexture(Framebuffer::Attachments::RGBA);
...
void MousePicker::updatePosition(unsigned int posX, unsigned int posY, const Camera& camera) {
	float mouseXndc = (2.0f * posX) / (float)Application::Width - 1.0f;
	float mouseYndc = 1.0f - (2.0f * posY) / (float)Application::Height;
