...
void MainMenuUI::RefreshWorlds(Game* game) {
	...
	for (auto& p : std::experimental::filesystem::directory_iterator("gamedir/worlds")) {
		std::string worldName = p.path().filename().generic_string();

		WorldMeta meta = WorldMeta::loadWorldMeta("gamedir/worlds/mainmenu");
...
void MainMenuUI::Setup() {
	...
	title = new Texture();
	title->loadFromFile("res/ui/temp_logo.png", false);
	setup = true;
}
...
void MainMenuUI::DrawMainMenu(Game* game) {
	...
	ImGui::SetNextWindowPos(ImVec2(Application::Width / 2 - 400 / 2, Application::Height / 2 - 300 / 2), ImGuiCond_FirstUseEver);
	...
	ImTextureID texId = (ImTextureID)title->getTexture();
	...
	ImGui::SetCursorPos(ImVec2((ImGui::GetWindowSize().x - title->getWidth() / 2) * 0.5f, ImGui::GetCursorPosY() + 10));
	ImGui::Image(texId, ImVec2(title->getWidth() / 2, title->getHeight() / 2), uv_min, uv_max, tint_col, border_col);
...
void MainMenuUI::DrawSingleplayer(Game* game) {
	...
	ImGui::SetNextWindowPos(ImVec2(Application::Width / 2 - 200, Application::Height / 2 - 250), ImGuiCond_FirstUseEver);
	if (!ImGui::Begin("Main Menu", NULL, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse)) {
		ImGui::End();
		return;
	}
...
void MainMenuUI::DrawMultiplayer(Game* game) {
	...
	if (ImGui::InputTextWithHint("", "ip:port", connectBuf, IM_ARRAYSIZE(connectBuf))) {
		...
		connectButtonEnabled = std::regex_match(str, connectInputRegex);
	}
	ImGui::PopItemWidth();
	ImGui::PopStyleVar();
	ImGui::SameLine();
