...
Texture* CreateWorldUI::materialTestWorld = nullptr;
Texture* CreateWorldUI::defaultWorld = nullptr;
...
void CreateWorldUI::Setup() {
	...
	materialTestWorld = new Texture();
	materialTestWorld->loadFromFile("res/ui/prev_materialtest.png", false);

	defaultWorld = new Texture();
	defaultWorld->loadFromFile("res/ui/prev_default.png", false);
...
void CreateWorldUI::Draw(Game* game) {
	...
	ImTextureID texId = (ImTextureID)materialTestWorld->getTexture();
	...
	// imgui_impl_opengl3.cpp implements ImTextureID as GLuint 
	texId = (ImTextureID)defaultWorld->getTexture();
	...
	if (ImGui::Button("##create", ImVec2(150, 36))) {
		...
		worldTitle = std::regex_replace(worldTitle, trimWhitespaceRegex, "$1");
...
void CreateWorldUI::inputChanged(std::string text, Game* game) {
	...
	std::string folder = "gamedir/worlds/mainmenu";
	struct stat buffer;
	...
	while (exists) {
		...
		folder = "gamedir/worlds/mainmenu";
