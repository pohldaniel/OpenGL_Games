...
void Chunk::read() {
	...
	uint32_t* background = new uint32_t[CHUNK_W * CHUNK_H];
...
void Chunk::write(MaterialInstance* tiles, MaterialInstance* layer2, uint32_t* background) {
    ...
    for(int i = 0; i < CHUNK_W * CHUNK_H; i++) {
        buf[i] = {(uint16_t)tiles[i].mat->id, tiles[i].color, tiles[i].temperature};
        buf[CHUNK_W * CHUNK_H + i] = {(uint16_t)layer2[i].mat->id, layer2[i].color, layer2[i].temperature};
    }
