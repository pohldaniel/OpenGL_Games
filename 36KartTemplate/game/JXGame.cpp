...
JXGame::JXGame() {
  ...
  std::shared_ptr<Geometry> terrainGeom = res.tryGetGeometry("ressources/DE_Map1/Landscape01.obj");
  ...
  physicsChunkManager = std::make_unique<PhysicsChunkManager>(terrainVerts, "ressources/chunk_map.txt");
}
    ...
    if (ImGui::BeginTabBar("DebugTabBar")) {
        ...
        if (ImGui::BeginTabItem("MISC")) {
            ...
            if (ImGui::Button("Export Terrain Chunk Map")) {
                ...
                RenderRsrcManager& res = renderer.get()->getRessourcePtr();
                std::shared_ptr<Geometry> terrainGeom = res.tryGetGeometry("ressources/Landscape01.obj");
                std::vector<GLfloat> terrainVerts = terrainGeom.get()->_vertices;

                GameDebugHelper::ExportVertexMappingToFile(terrainVerts, "ressources/cpp_vertex_mapping.txt");
