...
float shadowCoef() {
	...
	float ret = texture( stex, shadow_coord + vec4( offset, 0, 0)) * (pos.x) * (pos.y);
	ret += texture( stex, shadow_coord + vec4( offset.x, -offset.y, 0, 0)) * (pos.x) * (1-pos.y);
	ret += texture( stex, shadow_coord + vec4( -offset.x, offset.y, 0, 0)) * (1-pos.x) * (pos.y);
	ret += texture( stex, shadow_coord + vec4( -offset.x, -offset.y, 0, 0)) * (1-pos.x) * (1-pos.y);
