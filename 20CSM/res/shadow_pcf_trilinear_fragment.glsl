...
float shadowCoef()
{
	...
	if(gl_FragCoord.z < far_d[0]) {
		index = 0;
		blend = clamp( (gl_FragCoord.z - far_d[0] * 0.995) * 200.0, 0.0, 1.0); 
    }
	else if(gl_FragCoord.z < far_d[1]) {
		index = 1;
		blend = clamp( (gl_FragCoord.z - far_d[1] * 0.995) * 200.0, 0.0, 1.0); 
	}
	else if(gl_FragCoord.z < far_d[2]) {
		index = 2;
		blend = clamp( (gl_FragCoord.z - far_d[2] * 0.995) * 200.0, 0.0, 1.0); 
	}
	...
	float ret = texture(stex, shadow_coord);
	
	if (blend > 0.0) {
	    shadow_coord = u_shadow[index+1]*vPos;
	
	    shadow_coord.w = shadow_coord.z;
	    shadow_coord.z = float(index+1);
    	
	    ret = ret*(1.0-blend) + texture(stex, shadow_coord)*blend; 
	}
