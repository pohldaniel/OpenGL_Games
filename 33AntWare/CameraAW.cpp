...
Camera::Camera(float FOV) : FOV(FOV){
    aspectRatio = static_cast<float>(Application::Width) / static_cast<float>(Application::Height);
}
...
void Camera::update()
{
    ...
    mat4 viewPerspective = perspective(FOV, aspectRatio, 0.1f, 120.0f) * lookAt(camPos, lookedAt, camUp);
