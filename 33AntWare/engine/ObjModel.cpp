...
void ObjModel::loadSequence(const char* _path, bool isStacked, bool withoutNormals, bool generateSmoothNormals, bool generateFlatNormals, bool generateSmoothTangents, bool rescale) {
...
void ObjModel::loadSequence(const char* _path, Vector3f& axis, float degree, Vector3f& translate, float scale, bool isStacked, bool withoutNormals, bool generateSmoothNormals, bool generateFlatNormals, bool generateSmoothTangents, bool rescale) {
	
	m_modelDirectory = _path;
	...
	std::vector<int> numberOfTriangles;
	...
	for (int i = 0; i < numberOfTriangles.size(); i++) {
		m_meshes.push_back(new ObjMesh(numberOfTriangles[i], this));
		if (m_meshes.size() > 1) {
...
void ObjModel::drawRawSequence(unsigned short frame) {
	...
	//std::cout << "Count: " << m_meshes[frame]->m_drawCount << "  " << m_meshes[frame]->m_baseIndex << "  " << m_meshes[frame]->m_baseVertex << std::endl;
