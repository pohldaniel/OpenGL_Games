...
void Mesh::constructVAO(vector<shared_ptr<Mesh>> meshes){
	...
	for (unsigned i = 0; i < meshes.size(); ++i){
		...
		vertexBufferSize += meshes[i]->sizes[VERTEX_BUFFER_POS] + meshes[i]->sizes[VERTEX_BUFFER_TEX] +meshes[i]->sizes[VERTEX_BUFFER_NORM];

		meshes[i]->baseVertex = indicesOffset;
		indicesOffset += meshes[i]->vertices.size();
	}

	indexBufferSize = meshes[meshes.size() - 1]->offsets[INDEX_BUFFER] + meshes[meshes.size() - 1]->sizes[INDEX_BUFFER];
	texCoordsOffset = meshes[meshes.size() - 1]->offsets[VERTEX_BUFFER_POS] + meshes[meshes.size() - 1]->sizes[VERTEX_BUFFER_POS];
	normalsOffset = texCoordsOffset + meshes[meshes.size() - 1]->offsets[VERTEX_BUFFER_TEX] + meshes[meshes.size() - 1]->sizes[VERTEX_BUFFER_TEX];


	vertexBuffer.resize(vertexBufferSize);
	indexBuffer.resize(indexBufferSize);
	for (unsigned i = 0; i < meshes.size(); ++i){
		memcpy(vertexBuffer.data() + meshes[i]->offsets[VERTEX_BUFFER_POS], meshes[i]->vertices.data(), meshes[i]->sizes[VERTEX_BUFFER_POS]);
		memcpy(vertexBuffer.data() + meshes[i]->offsets[VERTEX_BUFFER_TEX] + texCoordsOffset, meshes[i]->texCoords.data(), meshes[i]->sizes[VERTEX_BUFFER_TEX]);
		memcpy(vertexBuffer.data() + meshes[i]->offsets[VERTEX_BUFFER_NORM] + normalsOffset, meshes[i]->normals.data(), meshes[i]->sizes[VERTEX_BUFFER_NORM]);
		memcpy(indexBuffer.data() + meshes[i]->offsets[INDEX_BUFFER], meshes[i]->indices.data(), meshes[i]->sizes[INDEX_BUFFER]);
	}
