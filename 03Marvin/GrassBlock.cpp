...
GrassBlock::GrassBlock(unsigned int category, const float& dt, const float& fdt) : Entity(category, dt, fdt) {
...
void GrassBlock::initAnimations() {
	m_textureAtlas = new unsigned int();
	m_currentFrame = new unsigned int();

	m_Animations["move"].create(Globals::spritesheetManager.getAssetPointer("grass_block"), 0.5f, *m_textureAtlas, *m_currentFrame);
}
