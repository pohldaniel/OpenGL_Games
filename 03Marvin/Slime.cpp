...
Slime::Slime(unsigned int category, const float& dt, const float& fdt) : Entity(category, dt, fdt) {
...
void Slime::initAnimations() {
	...
	m_Animations["move"].create(Globals::spritesheetManager.getAssetPointer("slime"), 0.5f, *m_textureAtlas, *m_currentFrame);
}
