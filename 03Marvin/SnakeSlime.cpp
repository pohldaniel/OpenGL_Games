...
SnakeSlime::SnakeSlime(unsigned int category, const float& dt, const float& fdt) : Entity(category, dt, fdt) {
...
void SnakeSlime::initAnimations() {
	m_textureAtlas = new unsigned int();
	m_currentFrame = new unsigned int();

	m_Animations["move"].create(Globals::spritesheetManager.getAssetPointer("snake_slime"), 0.5f, *m_textureAtlas, *m_currentFrame);
}

void SnakeSlime::update() {
	m_Animations["move"].update(m_dt);
}

void SnakeSlime::render() {
