...
void Person::CheckKick()
{
    ...
    if (AnimationLu::animations[victim->animTarget].height != lowheight) {
...
void Person::Reverse()
{
    ...
    if (victim->id == 0 && AnimationLu::animations[victim->animTarget].attack == reversal) {
        numreversals++;
    }
}
...
void Person::RagDoll(bool checkcollision)
{
    ...
    if (!skeleton.free) {
        ...
        if (AnimationLu::animations[animCurrent].height == lowheight || AnimationLu::animations[animTarget].height == lowheight) {
            skeleton.DoConstraints(&coords, &scale);
            skeleton.DoConstraints(&coords, &scale);
            skeleton.DoConstraints(&coords, &scale);
            skeleton.DoConstraints(&coords, &scale);
        ...
        for (unsigned i = 0; i < skeleton.joints.size(); i++) {
            if ((AnimationLu::animations[animCurrent].attack != reversed || animCurrent == swordslashreversedanim) && animCurrent != rabbitkickanim && !isLanding() && !wasLanding() && AnimationLu::animations[animCurrent].height == AnimationLu::animations[animTarget].height) {
...
void Person::DoAnimations()
{
    if (!skeleton.free) {
        ...
        if (animTarget == tempanim || animCurrent == tempanim) {
			AnimationLu::animations[tempanim] = tempanimation;
        }
        ...
        if (AnimationLu::animations[animTarget].attack != reversed) {
            feint = 0;
        }
        if (!crouchkeydown || (isLanding() || isLandhard()) || (wasLanding() || wasLandhard())) {
            ...
            if (!crouchtogglekeydown && AnimationLu::animations[animTarget].attack == reversed && isPlayerControlled() && (escapednum < 2 || reversaltrain)) {
                feint = 1;
            }
            if (!isFlip()) {
                crouchtogglekeydown = 1;
        ...
        if (AnimationLu::animations[animTarget].attack || animCurrent == getupfrombackanim || animCurrent == getupfromfrontanim) {
            if (detail) {
                normalsupdatedelay = 0;
            }
        }
        ...
        if (target >= 1) {
            ...
            if ((AnimationLu::animations[animTarget].attack == normalattack || animTarget == walljumprightkickanim || animTarget == walljumpleftkickanim) && (!feint) && (victim->skeleton.free != 2 || animTarget == killanim || animTarget == dropkickanim || animTarget == crouchstabanim || animTarget == swordgroundstabanim || animTarget == staffgroundsmashanim)) {
                if (animCurrent == spinkickanim && currentFrame().label == 5) {
                    if (distsq(&coords, &victim->coords) < (scale * 5) * (scale * 5) * 3 && AnimationLu::animations[victim->animTarget].height != lowheight) {
                        escapednum = 0;
                        if (id == 0) {
                            camerashake += .4;
                        }
                ...
                if (animCurrent == winduppunchanim && currentFrame().label == 5) {
                    if (distsq(&coords, &victim->coords) < (scale * 5) * (scale * 5) * 2) {
                        ...
                        if (victim->damage > victim->damagetolerance - 60 || normaldotproduct(victim->facing, victim->coords - coords) > 0 || AnimationLu::animations[victim->animTarget].height == lowheight) {
                            victim->RagDoll(0);
                        }
                        XYZ relative;
                        relative = victim->coords - coords;
                ...
                if (animCurrent == knifeslashstartanim && currentFrame().label == 5) {
                    if (hasvictim) {
                        if (distsq(&coords, &victim->coords) < (scale * 5) * (scale * 5) * 4.5 && victim->animTarget != dodgebackanim && victim->animTarget != rollanim) {
                            ...
                            //victim->jointVel(abdomen)+=relative*damagemult*200;
                            if (AnimationLu::animations[victim->animTarget].attack && (!victim->isPlayerControlled() || victim->animTarget == knifeslashstartanim) && (victim->creature == rabbittype || victim->deathbleeding <= 0)) {
                ...
                if (animCurrent == lowkickanim && currentFrame().label == 5) {
                    if (distsq(&coords, &victim->coords) < (scale * 5) * (scale * 5) * 3 && AnimationLu::animations[victim->animTarget].height != highheight) {
                        escapednum = 0;
                        if (id == 0) {
                            camerashake += .4;
                        }
                ...
                if (animCurrent == sweepanim && currentFrame().label == 5) {
                    if ((victim->animTarget != jumpupanim) &&
                        ...
                        if (AnimationLu::animations[victim->animTarget].height == middleheight || AnimationLu::animations[victim->animCurrent].height == middleheight || victim->damage >= victim->damagetolerance - 40) {
                            victim->RagDoll(0);
            ...
            if (AnimationLu::animations[animTarget].attack == reversal && (!victim->feint || (victim->lastattack == victim->lastattack2 && victim->lastattack2 == victim->lastattack3 && Random() % 2) || animTarget == knifefollowanim)) {
                ...
                if ((animTarget == swordslashreversalanim || animTarget == knifeslashreversalanim || animTarget == staffhitreversalanim || animTarget == staffspinhitreversalanim) && AnimationLu::animations[animTarget].frames[frameCurrent].label == 5) {
                ...
                if (hasvictim && (animCurrent == knifefollowanim || animCurrent == knifesneakattackanim) && currentFrame().label == 6) {
                    ...
                    if (hasWeapon() && AnimationLu::animations[victim->animTarget].attack != reversal) {
            ...
            //Animation end
            if (frameTarget >= int(AnimationLu::animations[animCurrent].frames.size())) {
                frameTarget = 0;
                if (wasStop()) {
                    animTarget = getIdle();
                    FootLand(leftfoot, 1);
                ...
                if (AnimationLu::animations[animTarget].attack == reversal && animCurrent != sneakattackanim && animCurrent != knifesneakattackanim && animCurrent != swordsneakattackanim && animCurrent != knifefollowanim) {
                ...
                if (AnimationLu::animations[animCurrent].attack == normalattack && !victim->skeleton.free && victim->animTarget != staggerbackhighanim && victim->animTarget != staggerbackhardanim && animTarget != winduppunchblockedanim && animTarget != blockhighleftanim && animTarget != swordslashparryanim && animTarget != swordslashparriedanim && animTarget != crouchstabanim && animTarget != swordgroundstabanim) {
                    animTarget = getupfromfrontanim;
                    lastfeint = 0;
                } else if (AnimationLu::animations[animCurrent].attack == normalattack) {
                    animTarget = getIdle();
                    lastfeint = 0;
                }
        ...
        if (!skeleton.free) {
            oldtarget = target;
            if (!transspeed && AnimationLu::animations[animTarget].attack != 2 && AnimationLu::animations[animTarget].attack != 3) {
                if (!isRun() || !wasRun()) {
                ...
                if (isRun() && wasRun()) {
                    ...
                    target += multiplier * AnimationLu::animations[animTarget].frames[frameCurrent].speed * speed * 1.7 * tempspeed / (speed * 45 * scale);
                }
            } else if (transspeed) {
            ...
            if (frameCurrent >= int(AnimationLu::animations[animCurrent].frames.size())) {
                frameCurrent = AnimationLu::animations[animCurrent].frames.size() - 1;
            }
...
void Person::DoStuff()
{
    ...
    if (deathbleeding > 0 && dead != 2) {
        ...
        if (bloodloss > damagetolerance && AnimationLu::animations[animTarget].attack == neutral) {
            if (hasWeapon()) {
    ...
    if (!skeleton.free) {
        ...
        if (AnimationLu::animations[animTarget].attack != reversal) {
            if (!isnormal(coords.x)) {
                coords = oldcoords;
            }
            oldcoords = coords;
        ...
        onterrain = 0;
        if (!isRun() && !AnimationLu::animations[animTarget].attack && animTarget != getupfromfrontanim && animTarget != getupfrombackanim && animTarget != sneakanim) {
            targettilt2 = 0;
        }
        if (animTarget == jumpupanim || animTarget == jumpdownanim || isFlip()) {
            flatvelocity = velocity;
        ...
        if (!AnimationLu::animations[animTarget].attack && animTarget != getupfrombackanim && animTarget != getupfromfrontanim) {
            if (tilt2 > 25) {
                tilt2 = 25;
            }
            if (tilt2 < -25) {
        ...
        if (animTarget == staggerbackhardanim && AnimationLu::animations[staggerbackhardanim].frames[frameTarget].label != 6) {
        ...
        if (isIdle() || animTarget == drawrightanim || animTarget == drawleftanim || animTarget == crouchdrawrightanim || animTarget == crouchstabanim || animTarget == swordgroundstabanim || isStop() || animTarget == removeknifeanim || animTarget == crouchremoveknifeanim || isLanding() || isCrouch() || AnimationLu::animations[animTarget].attack || (animTarget == rollanim && targetFrame().label == 6)) {
            velspeed = findLength(&velocity);
            velocity.y = 0;
        ...
        if (AnimationLu::animations[animTarget].attack == normalattack && animTarget != rabbitkickanim && !victim->skeleton.free) {
        ...
        if (AnimationLu::animations[animTarget].attack == reversal && animTarget != rabbittacklinganim) {
...
/* FUNCTION?
 */
int Person::SphereCheck(XYZ* p1, float radius, XYZ* p, XYZ* move, float* rotate, ModelLu* model)
{
...
int findPathDist(int start, int end)
{
    ...
    for (char i = 0; i < 50; i++) {
        ...
        while (last != end && count < 30) {
            closest = -1;
            for (int j = 0; j < GameLu::numpathpoints; j++) {
                if (j != last && j != last2 && j != last3 && j != last4) {
                    connected = 0;
                    if (GameLu::numpathpointconnect[j]) {
                        for (int k = 0; k < GameLu::numpathpointconnect[j]; k++) {
                            if (GameLu::pathpointconnect[j][k] == last) {
                                connected = 1;
                            }
                        }
                    }
                    if (!connected) {
                        if (GameLu::numpathpointconnect[last]) {
                            for (int k = 0; k < GameLu::numpathpointconnect[last]; k++) {
                                if (GameLu::pathpointconnect[last][k] == j) {
                                    connected = 1;
                                }
                            }
                        }
    ...
    if (!isPlayerControlled() && !Dialog::inDialog()) {
        ...
        if (aitype == pathfindtype) {
            if (finalpathfindpoint == -1) {
                ...
                closestdistance = -1;
                for (int j = 0; j < GameLu::numpathpoints; j++) {
                    if (closest == -1 || distsq(&finalfinaltarget, &GameLu::pathpoint[j]) < closestdistance) {
                        closestdistance = distsq(&finalfinaltarget, &GameLu::pathpoint[j]);
                        closest = j;
                        finaltarget = GameLu::pathpoint[j];
                    }
                }
                finalpathfindpoint = closest;
                for (int j = 0; j < GameLu::numpathpoints; j++) {
                    for (int k = 0; k < GameLu::numpathpointconnect[j]; k++) {
                        DistancePointLine(&finalfinaltarget, &GameLu::pathpoint[j], &GameLu::pathpoint[GameLu::pathpointconnect[j][k]], &tempdist, &colpoint);
                        if (sq(tempdist) < closestdistance) {
                            if (findDistance(&colpoint, &GameLu::pathpoint[j]) + findDistance(&colpoint, &GameLu::pathpoint[GameLu::pathpointconnect[j][k]]) <
                                findDistance(&GameLu::pathpoint[j], &GameLu::pathpoint[GameLu::pathpointconnect[j][k]]) + .1) {
                ...
                if (lastpathfindpoint == -1) {
                    for (int j = 0; j < GameLu::numpathpoints; j++) {
                        if (j != lastpathfindpoint) {
                            if (closest == -1 || (distsq(&coords, &GameLu::pathpoint[j]) < closestdistance)) {
                                closestdistance = distsq(&coords, &GameLu::pathpoint[j]);
                                closest = j;
                            }
                        }
                    }
                    ...
                    for (int j = 0; j < GameLu::numpathpoints; j++) {
                        if (j != lastpathfindpoint) {
                            for (int k = 0; k < GameLu::numpathpointconnect[j]; k++) {
                                DistancePointLine(&coords, &GameLu::pathpoint[j], &GameLu::pathpoint[GameLu::pathpointconnect[j][k]], &tempdist, &colpoint);
                                if (sq(tempdist) < closestdistance) {
                                    if (findDistance(&colpoint, &GameLu::pathpoint[j]) + findDistance(&colpoint, &GameLu::pathpoint[GameLu::pathpointconnect[j][k]]) <
                                        findDistance(&GameLu::pathpoint[j], &GameLu::pathpoint[GameLu::pathpointconnect[j][k]]) + .1) {
                                        closestdistance = sq(tempdist);
                    ...
                    for (int j = 0; j < GameLu::numpathpoints; j++) {
                        if (j != lastpathfindpoint &&
                            ...
                            if (GameLu::numpathpointconnect[j]) {
                                for (int k = 0; k < GameLu::numpathpointconnect[j]; k++) {
                                    if (GameLu::pathpointconnect[j][k] == lastpathfindpoint) {
                                        connected = 1;
                                    }
                                }
                            }
                            if (!connected) {
                                if (GameLu::numpathpointconnect[lastpathfindpoint]) {
                                    for (int k = 0; k < GameLu::numpathpointconnect[lastpathfindpoint]; k++) {
                                        if (GameLu::pathpointconnect[lastpathfindpoint][k] == j) {
                                            connected = 1;
                                        }
                                    }
                                }
            ...
            targetyaw = roughDirectionTo(coords, GameLu::pathpoint[targetpathfindpoint]);
            lookyaw = targetyaw;

            //reached target point
            if (distsqflat(&coords, &GameLu::pathpoint[targetpathfindpoint]) < .6) {
            ...
            if ((!Tutorial::active || cananger) &&
                ...
                if (distsq(&coords, &Person::players[0]->coords) < 12 &&
					AnimationLu::animations[Person::players[0]->animTarget].height != lowheight &&
                ...
                if (distsq(&coords, &Person::players[0]->coords) < 30 &&
					AnimationLu::animations[Person::players[0]->animTarget].height == highheight &&
                    !GameLu::editorenabled) {
                    aitype = attacktypecutoff;
                }

                if (losupdatedelay < 0 && !GameLu::editorenabled && occluded < 2) {
                    losupdatedelay = .2;
                    for (unsigned j = 0; j < Person::players.size(); j++) {
                        if (j == 0 || Person::players[j]->skeleton.free || Person::players[j]->aitype != passivetype) {
                            if (abs(Random() % 2) || AnimationLu::animations[Person::players[j]->animTarget].height != lowheight || j != 0) {
            ...
            if (aitype == attacktypecutoff && GameLu::musictype != 2) {
                if (creature != wolftype) {
                    stunned = .6;
                    surprised = .6;
                }
        ...
        if (aitype != passivetype && GameLu::leveltime > .5) {
            howactive = typeactive;
        }
        ...
        if (aitype == passivetype) {
            ...
            if (howactive < typesleeping &&
                ...
                occluded < 25) {
                if (distsq(&coords, &Person::players[0]->coords) < 12 &&
					AnimationLu::animations[Person::players[0]->animTarget].height != lowheight && !GameLu::editorenabled) {
                    aitype = attacktypecutoff;
                }
                if (distsq(&coords, &Person::players[0]->coords) < 30 &&
					AnimationLu::animations[Person::players[0]->animTarget].height == highheight && !GameLu::editorenabled) {
                    aitype = attacktypecutoff;
                }
                ...
                if (creature == wolftype) {
                    ...
                    for (unsigned j = 0; j < Person::players.size(); j++) {
                        if (j == 0 || (Person::players[j]->dead && Person::players[j]->bloodloss > 0)) {
                            ...
                            if (distsq(&coords, &windsmell) < smelldistance && !GameLu::editorenabled) {
                                aitype = attacktypecutoff;
                            }
                        }
                    }
                ...
                if (howactive < typesleeping && losupdatedelay < 0 && !GameLu::editorenabled && occluded < 2) {
                    losupdatedelay = .2;
                    for (unsigned j = 0; j < Person::players.size(); j++) {
                        if (j == 0 || Person::players[j]->skeleton.free || Person::players[j]->aitype != passivetype) {
                            if (abs(Random() % 2) || AnimationLu::animations[Person::players[j]->animTarget].height != lowheight || j != 0) {
                                    ...
                                    if (normaldotproduct(facing, Person::players[j]->coords - coords) > 0) {
                                        if ((-1 == Object::checkcollide(
                                            ...
                                            (Person::players[j]->animTarget == hanganim &&
                                             normaldotproduct(Person::players[j]->facing, coords - Person::players[j]->coords) < 0)) {
                                            lastseentime -= .2;
                                            if (j == 0 && AnimationLu::animations[Person::players[j]->animTarget].height == lowheight) {
                                                lastseentime -= .4;
                                            } else {
                                                lastseentime -= .6;
                                            }
        ...
        if (aitype == searchtype) {
            ...
            if (!Person::players[0]->dead &&
                ...
                if (abs(Random() % 2) || AnimationLu::animations[animTarget].height != lowheight) {
        ...
        if (aitype == attacktypecutoff) {
            ...
            if (damage < damagetolerance / 2) {
                if (AnimationLu::animations[animTarget].height != highheight) {
            ...
            //it's time to think (?)
            if (aiupdatedelay < 0 &&
                !AnimationLu::animations[animTarget].attack &&
                animTarget != staggerbackhighanim &&
        ...
        if (AnimationLu::animations[Person::players[0]->animTarget].height == highheight &&
            (aitype == attacktypecutoff ||
             aitype == searchtype)) {
