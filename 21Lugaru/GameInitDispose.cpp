...
//***************> ResizeGLScene() <******/
GLvoid GameLu::ReSizeGLScene(float fov, float pnear)
{
    if (screenheight == 0) {
        screenheight = 1;
    }
...
void GameLu::LoadScreenTexture()
{
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

    if (!GameLu::screentexture) {
        glGenTextures(1, &GameLu::screentexture);
    }
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);

    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, GameLu::screentexture);
...
void GameLu::LoadStuff()
{
    ...
    for (unsigned i = 0; i < Person::players[0]->skeleton.joints.size(); i++) {
        for (unsigned j = 0; j < AnimationLu::animations[knifesneakattackanim].frames.size(); j++) {
			AnimationLu::animations[knifesneakattackanim].frames[j].joints[i].position += moveamount;
        }
    }
    ...
    for (unsigned i = 0; i < Person::players[0]->skeleton.joints.size(); i++) {
        for (unsigned j = 0; j < AnimationLu::animations[knifesneakattackedanim].frames.size(); j++) {
			AnimationLu::animations[knifesneakattackedanim].frames[j].joints[i].position += moveamount;
        }
    }
    ...
    for (unsigned i = 0; i < Person::players[0]->skeleton.joints.size(); i++) {
		AnimationLu::animations[dead1anim].frames[1].joints[i].position = AnimationLu::animations[dead1anim].frames[0].joints[i].position;
		AnimationLu::animations[dead2anim].frames[1].joints[i].position = AnimationLu::animations[dead2anim].frames[0].joints[i].position;
		AnimationLu::animations[dead3anim].frames[1].joints[i].position = AnimationLu::animations[dead3anim].frames[0].joints[i].position;
		AnimationLu::animations[dead4anim].frames[1].joints[i].position = AnimationLu::animations[dead4anim].frames[0].joints[i].position;
    }
	AnimationLu::animations[dead1anim].frames[0].speed = 0.001;
	AnimationLu::animations[dead2anim].frames[0].speed = 0.001;
	AnimationLu::animations[dead3anim].frames[0].speed = 0.001;
	AnimationLu::animations[dead4anim].frames[0].speed = 0.001;

	AnimationLu::animations[dead1anim].frames[1].speed = 0.001;
	AnimationLu::animations[dead2anim].frames[1].speed = 0.001;
	AnimationLu::animations[dead3anim].frames[1].speed = 0.001;
	AnimationLu::animations[dead4anim].frames[1].speed = 0.001;

    for (unsigned i = 0; i < Person::players[0]->skeleton.joints.size(); i++) {
        for (unsigned j = 0; j < AnimationLu::animations[swordsneakattackanim].frames.size(); j++) {
			AnimationLu::animations[swordsneakattackanim].frames[j].joints[i].position += moveamount;
        }
    }
    LoadingScreen();
    for (unsigned j = 0; j < AnimationLu::animations[swordsneakattackanim].frames.size(); j++) {
		AnimationLu::animations[swordsneakattackanim].frames[j].weapontarget += moveamount;
    }

    LoadingScreen();

    for (unsigned i = 0; i < Person::players[0]->skeleton.joints.size(); i++) {
        for (unsigned j = 0; j < AnimationLu::animations[swordsneakattackedanim].frames.size(); j++) {
			AnimationLu::animations[swordsneakattackedanim].frames[j].joints[i].position += moveamount;
        }
    }
