...
int ModelLu::LineCheck(XYZ* p1, XYZ* p2, XYZ* p, XYZ* move, float* rotate)
{
...
int ModelLu::LineCheckPossible(XYZ* p1, XYZ* p2, XYZ* p, XYZ* move, float* rotate)
{
    static float distance;
    static float olddistance;
    static int intersecting;
...
int ModelLu::LineCheckSlidePossible(XYZ* p1, XYZ* p2, XYZ* move, float* rotate)
{
    static float distance;
    static float olddistance;
    static int intersecting;
...
int ModelLu::SphereCheck(XYZ* p1, float radius, XYZ* p, XYZ* move, float* rotate)
{
    static int i;
    static float distance;
    static float olddistance;
...
int ModelLu::SphereCheckPossible(XYZ* p1, float radius, XYZ* move, float* rotate)
{
    static float distance;
    static float olddistance;
    static int intersecting;
...
void ModelLu::UpdateVertexArray()
{
...
void ModelLu::UpdateVertexArrayNoTex()
{
    if (type != normaltype && type != decalstype) {
        return;
    }
...
void ModelLu::UpdateVertexArrayNoTexNoNorm()
{
    if (type != normaltype && type != decalstype) {
        return;
    }
...
bool ModelLu::loadnotex(const std::string& filename)
{
    FILE* tfile;
    long i;
    short triangleNum;
...
bool ModelLu::load(const std::string& filename)
{
    FILE* tfile;
    long i;
    short triangleNum;
...
bool ModelLu::loaddecal(const std::string& filename)
{
    FILE* tfile;
    long i, j;
    short triangleNum;
...
bool ModelLu::loadraw(const std::string& filename)
{
    FILE* tfile;
    long i;
    short triangleNum;
...
void ModelLu::ScaleTexCoords(float howmuch)
{
    for (unsigned int i = 0; i < Triangles.size(); i++) {
...
void ModelLu::Scale(float xscale, float yscale, float zscale)
{
    static int i;
    for (i = 0; i < vertexNum; i++) {
        vertex[i].x *= xscale;
...
void ModelLu::ScaleNormals(float xscale, float yscale, float zscale)
{
    if (type != normaltype && type != decalstype) {
        return;
    }
...
void ModelLu::Translate(float xtrans, float ytrans, float ztrans)
{
    static int i;
    for (i = 0; i < vertexNum; i++) {
        vertex[i].x += xtrans;
...
void ModelLu::Rotate(float xang, float yang, float zang)
{
    static int i;
    for (i = 0; i < vertexNum; i++) {
...
void ModelLu::CalculateNormals(bool facenormalise)
{
	GameLu::LoadingScreen();
...
void ModelLu::drawdecals(TextureLu shadowtexture, TextureLu bloodtexture, TextureLu bloodtexture2, TextureLu breaktexture)
{
    if (decalstoggle) {
        if (type != decalstype) {
            return;
...
void ModelLu::DeleteDecal(int which)
{
    if (decalstoggle) {
        if (type != decalstype) {
            return;
...
void ModelLu::MakeDecal(decal_type atype, XYZ* where, float* size, float* opacity, float* rotation)
{
    if (decalstoggle) {
        if (type != decalstype) {
            return;
...
const XYZ& ModelLu::getTriangleVertex(unsigned triangleId, unsigned vertexId) const
{
    return vertex[Triangles[triangleId].vertex[vertexId]];
}

void ModelLu::deleteDeadDecals()
{
    for (int i = decals.size() - 1; i >= 0; i--) {
