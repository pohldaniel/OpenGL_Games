...
void QuadTreeNew::setDefaults() {
	root = NULL;
	visibleGeometryNodes = NULL;
}

void QuadTreeNew::init(Vector3f* vertices, unsigned int* indices, unsigned int indicesCount, const Vector3f& min, const Vector3f& max, float minAABBSize) {
	destroy();
...
void QuadTreeNew::quickSortVisibleGeometryNodes(int left, int right) {
...
int QuadTreeNew::checkVisibility(bool sortVisibleGeometryNodes) {
...
void QuadTreeNew::draw(bool visualizeRenderingOrder) const {
...
void QuadTreeNew::drawAABB(int Depth) {
	if (root != NULL) {
		root->drawAABB(Depth);
	}
}

void QuadTreeNew::destroy() {
	if (root != NULL) {
