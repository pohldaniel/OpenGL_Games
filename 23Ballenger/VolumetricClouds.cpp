...
void VolumetricClouds::draw() {
	...
	if (model->postProcess) {
		...
		Shader2& cloudsPPShader = model->postProcessingShader->getShader();
		...
		cloudsPPShader.setVec2("resolution", glm::vec2(Application::Width, Application::Height));
		...
		cloudsPPShader.setFloat("time", time);
		if (!s->wireframe)
