...
Controls::Controls(StateMachine& machine) : State(machine, CurrentState::SETTINGS) {
	...
	m_textFields.at("rm").setPosition(static_cast<float>(Application::Width / 2 - 220), static_cast<float>(Application::Height - 400));
...
void Controls::render() {
	...
	Fontrenderer::Get().addText(Globals::fontManager.get("upheaval_50"), static_cast<float>(Application::Width / 2 - 220), static_cast<float>(Application::Height - 300), "Break", Vector4f(1.0f, 1.0f, 1.0f, 1.0f));
