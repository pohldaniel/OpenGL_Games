...
void Scene::RegisterObject()
{
    ...
    RegisterDerivedType<Scene, NodeTu>();
    CopyBaseAttributes<Scene, NodeTu>();
}
...
NodeTu* Scene::Instantiate(Stream& source)
{
    ZoneScoped;
...
NodeTu* Scene::InstantiateJSON(const JSONValue& source)
{
    ZoneScoped;
...
NodeTu* Scene::InstantiateJSON(Stream& source)
{
    ZoneScoped;
...
NodeTu* Scene::FindNode(unsigned id_) const
{
    auto it = nodes.find(id_);
    return it != nodes.end() ? it->second : nullptr;
}

void Scene::AddNode(NodeTu* node)
{
    ...
    if (node->NumChildren())
    {
        const std::vector<SharedPtr<NodeTu> >& nodeChildren = node->Children();
        for (auto it = nodeChildren.begin(); it != nodeChildren.end(); ++it)
            AddNode(*it);
    }
}

void Scene::RemoveNode(NodeTu* node)
{
    if (!node || node->ParentScene() != this)
        return;
