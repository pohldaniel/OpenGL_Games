    ...
    worldTransform(worldMatrixAllocator.Allocate())
{
    ...
    rotation = QuaternionTu::IDENTITY;
    scale = Vector3::ONE;
...
void SpatialNode::RegisterObject()
{
    ...
    RegisterDerivedType<SpatialNode, NodeTu>();
    CopyBaseAttributes<SpatialNode, NodeTu>();
    RegisterRefAttribute("position", &SpatialNode::Position, &SpatialNode::SetPosition, Vector3::ZERO);
    RegisterRefAttribute("rotation", &SpatialNode::Rotation, &SpatialNode::SetRotation, QuaternionTu::IDENTITY);
...
void SpatialNode::SetRotation(const QuaternionTu& newRotation)
{
...
{
    rotation = QuaternionTu(Vector3::FORWARD, newDirection);
    OnTransformChanged();
}
...
void SpatialNode::SetTransform(const Vector3& newPosition, const QuaternionTu& newRotation)
{
    position = newPosition;
    rotation = newRotation;
    OnTransformChanged();
...
void SpatialNode::SetTransform(const Vector3& newPosition, const QuaternionTu& newRotation, const Vector3& newScale)
{
    position = newPosition;
    rotation = newRotation;
    scale = newScale;
...
void SpatialNode::SetTransform(const Vector3& newPosition, const QuaternionTu& newRotation, float newScale)
{
...
void SpatialNode::SetWorldRotation(const QuaternionTu& newRotation)
{
    SpatialNode* parentNode = SpatialParent();
...
void SpatialNode::SetWorldTransform(const Vector3& newPosition, const QuaternionTu& newRotation)
{
    SpatialNode* parentNode = SpatialParent();
    if (parentNode)
    {
        Vector3 localPosition = parentNode->WorldTransform().Inverse() * newPosition;
		QuaternionTu localRotation = parentNode->WorldRotation().Inverse() * newRotation;
...
void SpatialNode::SetWorldTransform(const Vector3& newPosition, const QuaternionTu& newRotation, const Vector3& newScale)
{
    SpatialNode* parentNode = SpatialParent();
    if (parentNode)
    {
...
void SpatialNode::SetWorldTransform(const Vector3& newPosition, const QuaternionTu& newRotation, float newScale)
{
...
void SpatialNode::Rotate(const QuaternionTu& delta, TransformSpace space)
{
    switch (space)
    {
    case TS_LOCAL:
    ...
    case TS_WORLD:
        ...
        {
			QuaternionTu worldRotation = WorldRotation();
...
void SpatialNode::RotateAround(const Vector3& point, const QuaternionTu& delta, TransformSpace space)
{
    SpatialNode* parentNode = SpatialParent();
    Vector3 parentSpacePoint;
	QuaternionTu oldRotation = rotation;
...
{
    Rotate(QuaternionTu(angle, Vector3::UP), space);
}

void SpatialNode::Pitch(float angle, TransformSpace space)
{
    Rotate(QuaternionTu(angle, Vector3::RIGHT), space);
}

void SpatialNode::Roll(float angle, TransformSpace space)
{
    Rotate(QuaternionTu(angle, Vector3::FORWARD), space);
}
...
void SpatialNode::OnParentSet(NodeTu* newParent, NodeTu*)
{
