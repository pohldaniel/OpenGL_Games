...
class NodeTu : public Serializable
{
    OBJECT(NodeTu);
...
public:
    ...
    /// Return parent node.
    NodeTu* Parent() const { return parent; }
    ...
    NodeTu* Child(size_t index) const { return index < children.size() ? children[index] : nullptr; }
    /// Return all immediate child nodes.
    const std::vector<SharedPtr<NodeTu> >& Children() const { return children; }
    /// Return child nodes recursively.
    void FindAllChildren(std::vector<NodeTu*>& result) const;
    /// Return first child node that matches name.
    NodeTu* FindChild(const std::string& childName, bool recursive = false) const;
    /// Return first child node that matches name.
    NodeTu* FindChild(const char* childName, bool recursive = false) const;
    /// Return first child node that matches name hash.
    NodeTu* FindChild(StringHash nameHash, bool recursive = false) const;
    /// Return first child node of specified type.
    NodeTu* FindChildOfType(StringHash childType, bool recursive = false) const;
    /// Return first child node that matches type and name.
    NodeTu* FindChildOfType(StringHash childType, const std::string& childName, bool recursive = false) const;
    /// Return first child node that matches type and name.
    NodeTu* FindChildOfType(StringHash childType, const char* childName, bool recursive = false) const;
    /// Return first child node that matches type and name.
    NodeTu* FindChildOfType(StringHash childType, StringHash childNameHash, bool recursive = false) const;
    /// Return first child node that matches layer mask.
    NodeTu* FindChildByLayer(unsigned layerMask, bool recursive = false) const;
    /// Find child nodes of specified type.
    void FindChildren(std::vector<NodeTu*>& result, StringHash childType, bool recursive = false) const;
    /// Find child nodes that match layer mask.
    void FindChildrenByLayer(std::vector<NodeTu*>& result, unsigned layerMask, bool recursive = false) const;
...
protected:
    ...
    virtual void OnParentSet(NodeTu* newParent, NodeTu* oldParent);
...
protected:
    /// Child nodes.
    std::vector<SharedPtr<NodeTu> > children;
};
