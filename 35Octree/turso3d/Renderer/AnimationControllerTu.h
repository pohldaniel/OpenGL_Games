...
class AnimationControllerTu : public ObjectTu
{
	OBJECT(AnimationControllerTu);

public:
	/// Construct. Register subsystem
	AnimationControllerTu(AnimatedModelTu* model = nullptr);
	/// Destruct.
	~AnimationControllerTu();
	...
	AnimationStateTu* GetAnimationState(StringHash nameHash) const;
	AnimationStateTu* AddAnimationState(AnimationTu* animation);
	void RemoveAnimationState(AnimationStateTu* state);
	void FindAnimation(const std::string& name, unsigned& index, AnimationStateTu*& state) const;

	std::vector<AnimationControlTu> animations;
	std::vector<SharedPtr<AnimationStateTu> > nodeAnimationStates;

	AnimatedModelTu* model;
