...
public:
    ...
    void SetMaterial(size_t index, MaterialTu* material)
    {
        if (numGeometries < 2)
            ...
            *(reinterpret_cast<SharedPtr<MaterialTu>*>(geomPtr) + index * 2 + 1) = material;
    }
    ...
    MaterialTu* GetMaterial(size_t index) const
    {
        if (numGeometries < 2)
            return matPtr;
        else
            return *(reinterpret_cast<SharedPtr<MaterialTu>*>(geomPtr) + index * 2 + 1);
    }
...
class GeometryNode : public OctreeNodeTu
{
    OBJECT(GeometryNode);
...
public:
    ...
    void SetMaterial(MaterialTu* material);
    ...
    MaterialTu* GetMaterial(size_t index) const { return static_cast<GeometryDrawable*>(drawable)->batches.GetMaterial(index); }
    /// Return the draw call source data for direct access.
