...
public:
    ...
    void SetShader(ShaderTu* shader, const std::string& vsDefines = JSONValue::emptyString, const std::string& fsDefines = JSONValue::emptyString);
	...
	MaterialTu* Parent() const { return parent; }
    /// Return shader.
	ShaderTu* GetShader() const { return shader; }
...
class MaterialTu : public Resource
{
    OBJECT(MaterialTu);
...
public:
    ...
    /// Set a texture.
    void SetTexture(size_t index, TextureTu* texture);
	...
	TextureTu* GetTexture(size_t index) const { return textures[index]; }
    ...
    static MaterialTu* DefaultMaterial();
...
private:
    ...
    /// Material textures.
    SharedPtr<TextureTu> textures[MAX_MATERIAL_TEXTURE_UNITS];
    /// Uniform buffer.
    ...
    /// Default material.
    static SharedPtr<MaterialTu> defaultMaterial;
    /// All materials.
    static std::set<MaterialTu*> allMaterials;
...
inline ShaderProgram* Pass::GetShaderProgram(unsigned char programBits)
{
    ...
    else
    {
        ...
        ShaderProgram* newShaderProgram = shader->CreateProgram(
			MaterialTu::GlobalVSDefines() + parent->VSDefines() + vsDefines + geometryDefines[geomBits],
			MaterialTu::GlobalFSDefines() + parent->FSDefines() + fsDefines
        );
