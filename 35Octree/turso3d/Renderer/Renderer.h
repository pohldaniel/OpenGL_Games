...
struct ThreadOctantResult
{
    ...
    std::vector<AutoPtr<CollectBatchesTaskTu> > collectBatchesTasks;
...
private:
    ...
    void RenderBatches(CameraTu* camera, const BatchQueue& queue);
    ...
    AutoPtr<CullLightsTaskTu> cullLightsTasks[NUM_CLUSTER_Z];
    /// Face selection UV indirection texture 1.
    AutoPtr<TextureTu> faceSelectionTexture1;
    /// Face selection UV indirection texture 2.
    AutoPtr<TextureTu> faceSelectionTexture2;
    /// Cluster lookup 3D texture.
    AutoPtr<TextureTu> clusterTexture;
