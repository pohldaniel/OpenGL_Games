...
SourceBatches::~SourceBatches()
{
    if (numGeometries >= 2)
    {
        for (size_t i = 0; i < numGeometries; ++i)
            (reinterpret_cast<SharedPtr<MaterialTu>*>(geomPtr) + i * 2 + 1)->~SharedPtr<MaterialTu>();
    ...
    else
    {
        ...
        for (size_t i = 0; i < numGeometries; ++i)
        {
            *(reinterpret_cast<Geometry**>(geomPtr) + i * 2) = nullptr;
            new (reinterpret_cast<SharedPtr<MaterialTu>*>(geomPtr) + i * 2 + 1) SharedPtr<MaterialTu>();
        }
    }

    for (size_t i = 0; i < numGeometries; ++i)
        SetMaterial(i, MaterialTu::DefaultMaterial());
}
...
{
    RegisterDerivedType<GeometryNode, OctreeNodeTu>();
    CopyBaseAttributes<GeometryNode, OctreeNodeTu>();
    RegisterMixedRefAttribute("materials", &GeometryNode::MaterialsAttr, &GeometryNode::SetMaterialsAttr,
        ResourceRefList(MaterialTu::TypeStatic()));
}
...
void GeometryNode::SetMaterial(MaterialTu* material)
{
    if (!material)
        material = MaterialTu::DefaultMaterial();
...
void GeometryNode::SetMaterial(size_t index, MaterialTu* material)
{
    if (!material)
        material = MaterialTu::DefaultMaterial();
...
void GeometryNode::SetMaterialsAttr(const ResourceRefList& value)
{
    ...
    for (size_t i = 0; i < value.names.size(); ++i)
        SetMaterial(i, cache->LoadResource<MaterialTu>(value.names[i]));
}

ResourceRefList GeometryNode::MaterialsAttr() const
{
    ResourceRefList ret(MaterialTu::TypeStatic());
