...
class AnimatedModelTu;
class AnimatedModelDrawable;
class AnimationTu;
class AnimationStateTu;
class UniformBuffer;
...
class BoneTu : public SpatialNode
{
...
public:
    ...
    void SetTransformSilent(const Vector3& position_, const QuaternionTu& rotation_, const Vector3& scale_)
    {
        position = position_;
        rotation = rotation_;
        scale = scale_;
...
public:
    ...
    BoneTu* RootBone() const { return rootBone; }
    /// Return number of bones.
    size_t NumBones() const { return numBones; }
    /// Return all bone scene nodes.
    const AutoArrayPtr<BoneTu*>& Bones() const { return bones; }
    /// Return all animation states.
    const std::vector<SharedPtr<AnimationStateTu> >& AnimationStates() const { return animationStates; }
...
public:
    ...
    AnimationStateTu* AddAnimationState(AnimationTu* animation);
    /// Remove an animation by animation pointer.
    void RemoveAnimationState(AnimationTu* animation);
    ...
    void RemoveAnimationState(AnimationStateTu* state);
    ...
    /// Return the root bone.
    BoneTu* RootBone() const { return static_cast<AnimatedModelDrawable*>(drawable)->RootBone(); }
    /// Return number of bones.
    size_t NumBones() const { return static_cast<AnimatedModelDrawable*>(drawable)->NumBones(); }
    /// Return all bone scene nodes.
    const AutoArrayPtr<BoneTu*>& Bones() const { return static_cast<AnimatedModelDrawable*>(drawable)->Bones(); }
    /// Return all animation states.
    const std::vector<SharedPtr<AnimationStateTu> >& AnimationStates() const { return static_cast<AnimatedModelDrawable*>(drawable)->AnimationStates(); }
    ...
    AnimationStateTu* GetAnimationState(size_t index) const;
    /// Return animation state by animation pointer.
    AnimationStateTu* FindAnimationState(AnimationTu* animation) const;
    /// Return animation state by animation name.
    AnimationStateTu* FindAnimationState(const std::string& animationName) const;
    /// Return animation state by animation name.
    AnimationStateTu* FindAnimationState(const char* animationName) const;
    /// Return animation state by animation name hash.
    AnimationStateTu* FindAnimationState(StringHash animationNameHash) const;
