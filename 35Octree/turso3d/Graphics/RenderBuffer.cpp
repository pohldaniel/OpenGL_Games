...
bool RenderBuffer::Define(const IntVector2& size_, ImageFormat format_, int multisample_)
{
    ...
    if (multisample > 1)
        glRenderbufferStorageMultisample(GL_RENDERBUFFER, multisample, TextureTu::glInternalFormats[format], size.x, size.y);
    else
        glRenderbufferStorage(GL_RENDERBUFFER, TextureTu::glInternalFormats[format], size.x, size.y);
