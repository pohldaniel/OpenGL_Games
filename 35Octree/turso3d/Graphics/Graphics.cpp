...
void Graphics::SetFrameBuffer(FrameBufferTu* buffer)
{
    if (buffer)
        buffer->Bind();
    else
...
ShaderProgram* Graphics::CreateProgram(const std::string& shaderName, const std::string& vsDefines, const std::string& fsDefines)
{
    ...
    ShaderTu* shader = cache->LoadResource<ShaderTu>(shaderName);
    if (shader)
...
void Graphics::SetTexture(size_t index, TextureTu* texture)
{
    if (texture)
        texture->Bind(index);
    else
        TextureTu::Unbind(index);
}
...
void Graphics::Blit(FrameBufferTu* dest, const IntRect& destRect, FrameBufferTu* src, const IntRect& srcRect, bool blitColor, bool blitDepth, TextureFilterMode filter)
{
    FrameBufferTu::Bind(dest, src);
...
void RegisterGraphicsLibrary()
{
    ...
    TextureTu::RegisterObject();
    ShaderTu::RegisterObject();
