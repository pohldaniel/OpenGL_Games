...
SSSApproximation::SSSApproximation(StateMachine& machine) : State(machine, States::RAYMARCH) {
	...
	m_camera = Camera();
	m_camera.perspective(45.0f, static_cast<float>(Application::Width) / static_cast<float>(Application::Height), 1.0f, 1024.0f);
	m_camera.orthographic(0.0f, static_cast<float>(Application::Width), 0.0f, static_cast<float>(Application::Height), -1.0f, 1.0f);
	m_camera.lookAt(Vector3f(-15.5f, 20.0f, -15.5f), Vector3f(0.0f, 15.0f, 0.0f) + Vector3f(0.0f, 0.0f, 1.0f), Vector3f(0.0f, 1.0f, 0.0f));
...
void SSSApproximation::render() {
	...
	Globals::textureManager.get("statue").bind(0u);
	...
	m_irradiance.bindColorTexture(0u, 0u);
	...
	shader->loadMatrix("u_lightView", m_lightView);
	shader->loadVector("u_lightPos", m_lightPos);	
	shader->loadBool("u_useDiffuseColor", m_useDiffuse);
	shader->loadFloat("u_thikness", m_thikness);
	shader->loadFloat("u_minThikness", m_minThikness);
	...
	m_distance.bindColorTexture(0u, 2u);
...
void SSSApproximation::resize(int deltaW, int deltaH) {
	...
	m_irradiance.resize(Application::Width, Application::Height);
}
...
void SSSApproximation::renderUi() {
	...
	ImGui::SliderFloat("Thikness", &m_thikness, 0.0f, 1.0f);
	ImGui::SliderFloat("Min Thikness", &m_minThikness, 0.0f, 20.0f);
	ImGui::End();
