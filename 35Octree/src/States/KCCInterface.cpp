...
void KCCInterface::update() {
	...
	Vector3f pos = m_character->getWorldPosition();
	...
	if (move || dx != 0.0f || dy != 0.0f) {
		if (dx || dy) {			
			...
			m_character->rotate(0.0f, -dx * m_rotationSpeed, 0.0f);
		}
	}

	btVector3 postion = m_character->getBtPosition();
...
void KCCInterface::render() {
	...
	!m_overview ? m_character->draw(m_camera) : m_character->drawOverview(m_camera.getOrthographicMatrix(), m_view);
...
void KCCInterface::createScene() {
	...
	m_lift = new Lift();
	m_lift->initialize(shapeNode, m_kinematicLift, shapeNode->getWorldPosition() + Vector3f(0, 6.8f, 0), m_root->findChild<ShapeNode>("button"), m_liftButtonTrigger, m_kinematicLiftTrigger);
	m_kinematicLift->setUserPointer(shapeNode);
	m_kinematicLiftTrigger->setUserPointer(shapeNode);
