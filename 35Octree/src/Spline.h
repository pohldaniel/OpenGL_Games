...
public:
	...
	Spline(const std::vector<Vector3f>& knots, InterpolationMode mode = BEZIER_CURVE);
	/// Copy constructor.
	...
	const std::vector<Vector3f>& GetKnots() const { return knots_; }

	/// Return the knot at the specific index.
	Vector3f GetKnot(unsigned index) const { return knots_[index]; }

	/// Return the T of the point of the spline at f from 0.f - 1.f.
	Vector3f GetPoint(float f) const;

	/// Set the interpolation mode.
	void SetInterpolationMode(InterpolationMode interpolationMode) { interpolationMode_ = interpolationMode; }

	/// Set the knots of the spline.
	void SetKnots(const std::vector<Vector3f>& knots) { knots_ = knots; }

	/// Set the value of an existing knot.
	void SetKnot(const Vector3f& knot, unsigned index);
	/// Add a knot to the end of the spline.
	void AddKnot(const Vector3f& knot);
	/// Add a knot to the spline at a specific index.
	void AddKnot(const Vector3f& knot, unsigned index);
...
private:
	...
	Vector3f BezierInterpolation(const std::vector<Vector3f>& knots, float t) const;
	/// Perform Spline interpolation on the spline.
	Vector3f CatmullRomInterpolation(const std::vector<Vector3f>& knots, float t) const;
	/// Perform linear interpolation on the spline.
	Vector3f LinearInterpolation(const std::vector<Vector3f>& knots, float t) const;
	/// Linear interpolation between two Variants based on underlying type.
	Vector3f LinearInterpolation(const Vector3f& lhs, const Vector3f& rhs, float t) const;

	/// Interpolation mode.
	InterpolationMode interpolationMode_;
	/// Knots on the spline.
	std::vector<Vector3f> knots_;
};