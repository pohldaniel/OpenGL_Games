...
btCollisionShape* PhysicsCar::CreateVehicleShape()
{
	...
	btTransform localTrans = Physics::BtTransform(Vector3f(0, m_cfg.bodyConnectionToChasisHeight, 0));
...
void PhysicsCar::Initialize(btDiscreteDynamicsWorld *refWorld, const btTransform & trans){
	...
	m_carChassis = Physics::CreateRigidBody(m_cfg.mass, m_initialTrans, CreateVehicleShape());
	//m_carChassis->setMassProps(m_cfg.mass, btVector3(0.0f, 0.0f, 0.0f));
	m_refDynamicsWorld->addRigidBody(m_carChassis, Physics::collisiontypes::CAR, Physics::collisiontypes::TERRAIN);
	m_carChassis->setDamping(m_cfg.linearDamping, m_cfg.angularDamping);

	/// create vehicle
	m_vehicleRayCaster = new btFilteredVehicleRaycaster(m_refDynamicsWorld, Physics::collisiontypes::CAR, Physics::collisiontypes::TERRAIN);
