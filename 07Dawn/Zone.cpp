...
int Zone::deleteNPC(int x, int y) {
	for (unsigned int t = 0; t < m_npcs.size(); t++) {
		if ((m_npcs[t]->getXPos() + m_npcs[t]->m_characterType.getTileSet(Enums::ActivityType::Walking, Enums::Direction::S).getAllTiles()[0].textureRect.width > x) &&
			(m_npcs[t]->getXPos() < x)) {
			if ((m_npcs[t]->getYPos() + m_npcs[t]->m_characterType.getTileSet(Enums::ActivityType::Walking, Enums::Direction::S).getAllTiles()[0].textureRect.height > y) &&
				(m_npcs[t]->getYPos() < y)) {
				removeNPC(m_npcs[t]);
				return 0;
			}
...
bool Zone::findInteractionPointForCharacter(Character *character) const {
	for (size_t curIP = 0; curIP < m_interactionPoints.size(); curIP++) {
		CharacterInteractionPoint *curCharacterIP = dynamic_cast<CharacterInteractionPoint*>(m_interactionPoints[curIP]);
