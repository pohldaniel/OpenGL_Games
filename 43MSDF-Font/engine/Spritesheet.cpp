...
void Spritesheet::loadFromFile(std::string fileName, const bool _flipVertical, unsigned int _internalFormat, unsigned int _format, unsigned int SOIL_FLAG) {
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D_ARRAY, m_texture);
	glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, m_internalFormat, m_width, m_height, m_totalFrames, 0, m_format, m_type, NULL);
	...
	glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, 0, m_width, m_height, 1, m_format, m_type, imageData);
...
void Spritesheet::LoadFromFile(unsigned int& textureRef, std::string fileName, const bool flipVertical, unsigned int internalFormat, unsigned int format, unsigned int SOIL_FLAG) {
	int width, height, numComponents;
	unsigned char* imageData = SOIL_load_image(fileName.c_str(), &width, &height, &numComponents, SOIL_FLAG);

	if (numComponents == 1 && (format == GL_RGB || format == GL_RGBA /*|| SOIL_FLAG == 1u*/)) {
		SOIL_free_image_data(imageData);
		SOIL_FLAG = format == GL_RGB ? 3u : 4u;
		imageData = SOIL_load_image(fileName.c_str(), &width, &height, 0, SOIL_FLAG);
		numComponents = format == GL_RGB ? 3 : 4;
	}

	unsigned int _internalFormat = internalFormat == 0 && numComponents == 1 ? GL_R8 : internalFormat == 0 && numComponents == 3 ? GL_RGB8 : internalFormat == 0 ? GL_RGBA8 : internalFormat;
	unsigned int _format = format == 0 && numComponents == 1 ? GL_R : format == 0 && numComponents == 3 ? GL_RGB : format == 0 ? GL_RGBA : format;
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D_ARRAY, textureRef);
	glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, _internalFormat, width, height, 1, 0, _format, GL_UNSIGNED_BYTE, NULL);
	...
	glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, 0, width, height, 1, _format, GL_UNSIGNED_BYTE, imageData);
