...
eQuat rotateCounterClockwiseZ(0.0f, 0.0f, std::sinf(DEG2RAD(ROTATION_INCREMENT) / 2.0f), std::cosf(DEG2RAD(ROTATION_INCREMENT) / 2.0f));
eQuat rotateClockwiseZ(0.0f, 0.0f, std::sinf(DEG2RAD(-ROTATION_INCREMENT) / 2.0f), std::cosf(DEG2RAD(-ROTATION_INCREMENT) / 2.0f));
...
void eVec2::Rotate(float degrees, const eVec2 & origin) {
	const float cosAngle = std::cosf(DEG2RAD(degrees));
	const float sinAngle = std::sinf(DEG2RAD(degrees));
