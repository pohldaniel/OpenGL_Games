...
void eBlendState::Init() {
	currentFrame = &animations[currentAnimationIndex]->getAnimationFrames()[0];
	duration = (animations[currentAnimationIndex]->getDuration() / speed) + 16.666f;	// BUGFIX: + FixedTime() prevents skipping the last animation frame during playback
}
...
bool eBlendState::AddBlendNode(const std::string& animationName, float xPosition, float yPosition) {
	...
	Animation& animation = AnimationManager::Get().getAnimation(animationName);
	//if (!animation->IsValid())
	//return false;

	animations.emplace_back(std::shared_ptr<Animation>(&animation));
	blendNodesHash.Add(std::hash<std::string>()(animationName), index);
	blendNodes.emplace_back(Vector2f(xPosition, yPosition));
	return true;
}
...
void eBlendState::Update() {
	...
	Vector2f controllerNode(xBlend, (blendMode == AnimationBlendMode::SIMPLE_1D ? 0.0f : yBlend));
	...
	for (size_t i = 0; i < blendNodes.size(); ++i) {
		float distSqr = (blendNodes[i] - controllerNode).lengthSq();
