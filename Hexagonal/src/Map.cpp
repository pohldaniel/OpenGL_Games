	...
	if (event.button == 1u) {
		...
		for (int j = 0; j < m_layer.size(); j++) {
			if (isValid(row, col) && m_layer[j][col][row].first != -1) {
				...
				Cell2& cell = m_selectedCells.back();
				...
				if (cell.selected && cell.visible) {
					...
					m_singleCache.erase(std::remove_if(m_singleCache.begin(), m_singleCache.end(), [](SingleSelectedCell2 cell) {return cell.found; }), m_singleCache.end());
				}
			}
		}
		m_mouseDown = true;
...
void Map::OnMouseMotion(Event::MouseMoveEvent& event) {
	...
	if (m_mouseDown) {
		...
		if (m_selectionMode != SelectionMode2::MARKER)
		...
		if (m_selectionMode == SelectionMode2::RASTERIZER) {
			...
			for (auto point : points) {
				for (int j = 0; j < m_layer.size(); j++) {
					if (isValid(point[0], point[1]) && m_layer[j][point[1]][point[0]].first != -1) {
						...
						SingleSelectedCell2 defaultCell = { point[0], point[1], false };
						std::vector<SingleSelectedCell2>::iterator it = std::find_if(m_singleCache.begin(), m_singleCache.end(), std::bind(FindSingleCell, std::placeholders::_1, defaultCell));
		...
		}else if (m_selectionMode == SelectionMode2::BOXSELECTION) {
			//box selection
				...
				for (int y = bottom; y < top; y = y + cellHeight * 0.5f * m_zoomFactor) {
					...
					for (int j = 0; j < m_layer.size(); j++) {
						if (isValid(row, col) && m_layer[j][col][row].first != -1) {
							...
							SingleSelectedCell2 defaultCell = { row, col, false };
							std::vector<SingleSelectedCell2>::iterator it = std::find_if(m_singleCache.begin(), m_singleCache.end(), std::bind(FindSingleCell, std::placeholders::_1, defaultCell));
							if (it != m_singleCache.end())
								it->found = true;
						}
		...
		}else if (m_selectionMode == SelectionMode2::ISOSELECTION) {
			// iso selection
			int row1, col1;
		...
		}else if (m_selectionMode == SelectionMode2::MARKER) {
			//marker
			int row, col;
...
void Map::OnMouseButtonUp(Event::MouseButtonEvent& event) {
	...
	if (event.button == 1u) {
		...
		m_singleCache.erase(std::remove_if(m_singleCache.begin(), m_singleCache.end(), [this](SingleSelectedCell2 cell) {
...
void Map::processCache(std::vector<std::reference_wrapper<Cell2>>& cache, bool visible, bool selected, bool clear) {
	std::for_each(cache.begin(), cache.end(), [visible, selected](std::reference_wrapper<Cell2> const& cell) {
...
void Map::processCache(std::vector<std::reference_wrapper<Cell2>>& cache, bool visible, bool selected, std::vector<std::reference_wrapper<Cell2>>& storage, bool clearAfterCopy) {
	std::for_each(cache.begin(), cache.end(), [visible, selected](std::reference_wrapper<Cell2> const& cell) {
		cell.get().selected = selected;
		cell.get().visible = visible;
...
bool Map::FindSingleCell(SingleSelectedCell2 const& s1, SingleSelectedCell2 const& s2) {
