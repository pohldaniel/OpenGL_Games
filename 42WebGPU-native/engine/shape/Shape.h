...
public:
	...
	void buildQuadXY(const  std::array<float, 3>& position = { -1.0f, -1.0f, 0.0f }, const std::array<float, 2>& size = { 2.0f, 2.0f }, unsigned int uResolution = 1, unsigned int vResolution = 1, bool generateTexels = true, bool generateNormals = true, bool generateTangents = false);
	void buildQuadXZ(const  std::array<float, 3>& position = { -1.0f, 0.0f, -1.0f }, const std::array<float, 2>& size = { 2.0f, 2.0f }, unsigned int uResolution = 1, unsigned int vResolution = 1, bool generateTexels = true, bool generateNormals = true, bool generateTangents = false);
