...
public:
	...
	void buildCylinder(const std::array<float, 3>& position = { 0.0f, 0.0f, 0.0f }, float baseRadius = 1.0f, float topRadius = 1.0f, float length = 1.0f, bool top = true, bool bottom = true, unsigned int uResolution = 10u, unsigned int vResolution = 10u, bool generateTexels = true, bool generateNormals = true, bool generateTangents = false);
