...
RenderBundles::RenderBundles(StateMachine& machine) : State(machine, States::RENDER_BUNDLES) {
	...
	wgpContext.createRenderPipeline("OCCLUSION", "RP_OCCLUSION", VL_PNC, std::bind(&RenderBundles::OnBindGroupLayouts, this));
...
WGPUQuerySet RenderBundles::createQuerySet() {
...
float RenderBundles::PingPongSine(float t) {
	return sinf(t * TWO_PI) * 0.5f + 0.5f;
}

void RenderBundles::InitScene(Scene& scene, Shape& shape, const WgpBuffer& uniformBuffer, std::array<float, 4> color) {
...
void RenderBundles::UpdateScene(Scene& scene, const Vector3f& position, float time) {
	Transform transform;
...
void RenderBundles::OnMapBuffer(WGPUMapAsyncStatus status, WGPUStringView message, void* userdata1, void* userdata2) {
