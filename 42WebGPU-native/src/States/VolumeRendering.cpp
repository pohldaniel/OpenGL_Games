...
VolumeRendering::VolumeRendering(StateMachine& machine) : State(machine, States::DEFFERED_RENDERING) {
	...
	wgpContext.createRenderPipeline("DEFFERED", "RP_DEFFERED", VL_NONE, std::bind(&VolumeRendering::OnBindGroupLayoutsDeffered, this));

	wgpContext.addSahderModule("DEFFERED_DEBUG", "res/shader/deffered_debug.wgsl");
	wgpContext.createRenderPipeline("DEFFERED_DEBUG", "RP_DEFFERED_DEBUG", VL_NONE, std::bind(&VolumeRendering::OnBindGroupLayoutsDefferedDebug, this),
		1u,
	...
	wgpContext.createRenderPipeline("GBUFFER", "RP_GBUFFER", VL_PTN, std::bind(&VolumeRendering::OnBindGroupLayoutsGBuffer, this),
		1u,
		WGPUPrimitiveTopology_TriangleList,
	...
	wgpContext.createComputePipeline("COMPUTE", "main", "CP_DEFFERED", std::bind(&VolumeRendering::OnBindGroupLayoutsCompute, this));
	...
	m_wgpDragon.setBindGroups("BG", std::bind(&VolumeRendering::OnBindGroupsGBuffer, this));

	m_wgpQuad.create(m_quad);
	m_wgpQuad.setBindGroups("BG", std::bind(&VolumeRendering::OnBindGroupsGBuffer, this));
...
std::vector<WGPUBindGroupLayout> VolumeRendering::OnBindGroupLayoutsGBuffer() {
...
std::vector<WGPUBindGroupLayout> VolumeRendering::OnBindGroupLayoutsCompute() {
	std::vector<WGPUBindGroupLayout> bindingLayouts(1);
...
std::vector<WGPUBindGroupLayout> VolumeRendering::OnBindGroupLayoutsDeffered() {
...
std::vector<WGPUBindGroupLayout> VolumeRendering::OnBindGroupLayoutsDefferedDebug() {
	std::vector<WGPUBindGroupLayout> bindingLayouts(1);
...
std::vector<WGPUBindGroup> VolumeRendering::OnBindGroupsGBuffer() {
...
WGPUBindGroup VolumeRendering::createDefferedBindGroup() {
...
WGPUBindGroup VolumeRendering::createLightBindGroup() {
	std::vector<WGPUBindGroupEntry> bindGroupEntries(3);
...
WGPUBindGroup VolumeRendering::createComputeBindGroup() {
	std::vector<WGPUBindGroupEntry> bindGroupEntries(3);
...
float VolumeRendering::randomFloat(float min, float max) {
